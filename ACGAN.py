...
class ACGAN(object):
    ...
    def __init__(self, sess, epoch, batch_size, z_dim, dataset_name, checkpoint_dir, result_dir, log_dir):
        ...
        if dataset_name == 'mnist' or dataset_name == 'fashion-mnist':
            ...
            self.input_width = 25
            self.output_height = 25
            self.output_width = 25
            self.input_depth = 25
            self.output_depth = 25

            self.z_dim = z_dim         # dimension of noise-vector
            self.y_dim = 1         # dimension of code-vector (label)
            self.c_dim = 1
            ...
            # code
            self.len_discrete_code = 1  # categorical distribution (i.e. label)
            self.len_continuous_code = 2  # gaussian distribution (e.g. rotation, thickness)
    ...
    def discriminator(self, x, is_training=True, reuse=False):
        ...
        with tf.variable_scope("discriminator", reuse=reuse):
            ...
            net = lrelu(conv3d(x, 64, 4, 4, 2, 2, name='d_conv1'))
            net = lrelu(bn(conv3d(net, 128, 4, 4, 2, 2, name='d_conv2'), is_training=is_training, scope='d_bn2'))
    ...
    def generator(self, z, y, is_training=True, reuse=False):
        ...
        with tf.variable_scope("generator", reuse=reuse):
            ...
            net = tf.nn.relu(
                bn(deconv3d(net, [self.batch_size, 14, 14, 64], 4, 4, 2, 2, name='g_dc3'), is_training=is_training,
                   scope='g_bn3'))

            out = tf.nn.sigmoid(deconv3d(net, [self.batch_size, 25, 25, 25, 1], 4, 4, 2, 2, name='g_dc4'))

            return out

    def build_model(self):
        # some parameters
        image_dims = [self.input_height, self.input_width, self.input_depth, self.c_dim]
        bs = self.batch_size
